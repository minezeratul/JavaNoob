...
public class MyBST<E extends Comparable<E>> {
    ...
    public boolean delete(E element){
        ...
        if (cur.left == null) {
            ...
            if (parent == null) { //the deleted node is root , so it needs to connect its rightChild
                root = cur.right;
            }
            else {
                if (element.compareTo(parent.element) < 0)
            ...
            if (parentOfRightMost.right == rightMost)
                ...
                // which means tree deleted cur.left.
                // and pofRightmost needs to connect its left node
    ...
    public void depthFirstTraversal(){ //non recursive postorder
        if (this.root == null)
            return;
    ...
    public boolean isFull(TreeNode root){
        ...
        if (root.left == null && root.right == null)//non-leaves, keep going down
            return true;
        else
    ...
    public int minDepth(){//use a queue
        LinkedList<TreeNode> res = new LinkedList<>();
        if (root == null)
            return 0;

        res.offer(this.root);
        return minDepth(res , 1);
    }


    public int minDepth(LinkedList<TreeNode> list , int depth){
        int size = list.size();
