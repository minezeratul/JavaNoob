...
public class Test7 {
    ...
    public static Pair distance(Point[] pointsOrderedOnX, int low, int high, Point[] pointsOrderedOnY) {
        ...
        for (Point point : pointsOrderedOnY)
            if (point.x <= pointsOrderedOnX[mid].x
                    && point.x >= pointsOrderedOnX[mid].x - d)
                stripL.add(point);
            else
                stripR.add(point);
